Game.achievementsUI = (function(){
    ...
    instance.initialise = function() {
        ...
        this.entryTemplate = Handlebars.compile(
            ['<div id="{{id}}" class="achievementTD btn" style="border:none; border-radius: 0px;">',
                '<div id="{{id}}_div" data-toggle="tooltip" title="{{title}}" style="padding: 15px; border:2px solid white;">',
                '<div id="{{id}}_bg" style="width: 50px; height: 40px; background: url({{iconPath}}{{iconName}}.{{iconExtension}}) no-repeat center; -webkit-background-size: contain;background-size: contain; margin: auto;opacity: 0.2"></div>',
    ...
    instance.createCategoryRow = function (category) {
        ...
        data.colc = 0;
        data.col = $('<div class="btn-group btn-group-justified" role="group"></div>');
